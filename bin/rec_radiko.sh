...
if [ $# -le 1 ]; then
  echo "usage : $0 channel_name duration(minuites) output-file"
  exit 1
fi
...
if [ "$(uname)" = "Darwin" ]; then
    ffmpeg -loglevel quiet -y -i "/tmp/${channel}_${date}" -acodec libmp3lame -ab 128k "${outfile}"
else
    avconv -loglevel quiet -y -i "/tmp/${channel}_${date}" -acodec libmp3lame -ab 128k "${outfile}"
fi
